...
public class EnhancedBoardEvaluator implements BoardEvaluator {
    ...
    @Override
    public int evaluate(final Board board, BotMove4 bot) {
        return board.getCurrentPlayer().isInStaleMate() || isRepetition(bot) ? 0 : // position is equal if in stalemate (also discourages repetitions)
    ...
    private static int castleValue(Board board) {
        return board.getCurrentPlayer().isCastled() ? CASTLE_BONUS : 0;
    }

    private static boolean isRepetition(BotMove4 bot) {
        return bot.getLastMove() != Move.NULL_MOVE && bot.getLastMove() == bot.getMove();
    }
